...
PxMATRIX display(128, 64, P_LAT, P_OE, P_A, P_B, P_C, P_D);
...
const int DisplayS = 16;
const int TextSize = 2;

// Function to refresh the display
void display_updater() {
  display.display(refreshRate); // Refresh rate
}
...
void setup() {
  ...
  display.begin(DisplayS); // Brightness level
  display.clearDisplay(); // Clear the display
  display_ticker.attach(0.004, display_updater);

  // Set text color and size
  display.setTextSize(TextSize); // Font size
